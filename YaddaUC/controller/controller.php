...
class Controller {
    ...
    public function doSomething() {
      ...
      case 'U':   //user create
                ...
                if (isset($_POST)) {
                    $this->createUser($_POST, $_FILES);               // activate controller
                }
                $view1->display();
                break;
                case 'Ub':   //user activation create
                $this->model = new Admin(null, null, null, null, null, null, null); // init a model
    ...
    public function activateUser($p) {
        if (isset($p) && count($p) > 0) {
            $user = Admin::createObject($p);  // object from array
